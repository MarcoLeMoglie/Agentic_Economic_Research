...
% -------- Slide 1: Intro --------
\begin{frame}{What you will see in detail next session}
...
\begin{frame}{Where agents fit across the pipeline}
  ...
  \begin{tikzpicture}[every node/.style={align=center,font=\scriptsize}]
    ...
    \node[below=0.15cm of idea,color=muted] {low};
    \node[below=0.15cm of lit,color=accent] {medium};
    \node[below=0.15cm of data,color=good] {high};
    \node[below=0.15cm of ana,color=accent] {medium};
    \node[below=0.15cm of write,color=good] {high};
    \node[below=0.15cm of rev,color=accent] {medium};
  \end{tikzpicture}
  \end{center}
  \vspace{0.3cm}
  \begin{center}\small Colour = how trustworthy agents are at each phase today.\end{center}
  \vspace{0.4cm}
...
\begin{frame}{What we will actually do in Session 2}
  \vspace{0.2cm}
  \threebullets%
    {Walk through \emph{my own} replication packages, with the agent live on screen.}%
    {Show prompts that work and prompts that fail, with their cost.}%
    {Compare manual vs agent timings on the same task, no marketing claims.}
  \vspace{0.6cm}
  \begin{center}\Large\color{deepblue}\bfseries Bring a research project you would like to test on.\end{center}
\end{frame}
