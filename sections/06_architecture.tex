...
\begin{frame}{Memory has layers}
  ...
  \begin{tikzpicture}[every node/.style={align=center,font=\small}]
    ...
    \node[draw=deepblue,fill=softgreen,rounded corners,very thick,
          ...
          (pers){\textbf{Persistent}\\\scriptsize across sessions};
...
\begin{frame}{MCP vs API vs CLI}
  ...
  \toprule
  API & A programmatic endpoint: \emph{your code} calls a service over HTTP.\\
  CLI & A command-line tool: \emph{a human or an agent} runs commands.\\
  MCP & The \emph{Model Context Protocol}: a standard way to expose tools, files, and data sources to agents so the agent can discover and call them \citep{anthropic_mcp_2024}.\\
  \bottomrule
  \end{tabularx}
  \end{center}
  \vspace{0.4cm}
...
\begin{frame}{Skills, hooks, loops}
  \begin{columns}[T,onlytextwidth]
    \column{0.32\textwidth}
      \textbf{Skills}\\[0.15cm]
      Reusable procedures: ``how we make a figure'', ``how we check tables'', ``how we write a referee response''.
    \column{0.32\textwidth}
      \textbf{Hooks}\\[0.15cm]
      Automatic checks before or after an agent acts: lint, format, prose-check, test.
    \column{0.32\textwidth}
      \textbf{Loops}\\[0.15cm]
      Recurring tasks: weekly literature scan, monthly replication check, post-revision audit.
  \end{columns}
  \vspace{0.6cm}
