...
\begin{frame}{Where you actually meet agentic AI}
  ...
  \begin{tikzpicture}[every node/.style={align=center,font=\small}]
    ...
    \node[draw=deepblue,fill=softgreen,rounded corners,very thick,
          ...
          (ide){\textbf{3. IDE agent}\\\scriptsize Cursor, Copilot, Windsurf, Antigravity, Codex\\\scriptsize code, refactor, test};
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      \threebullets%
        ...
        {Weakness: limited file control, hidden limits, reproducibility, AI sloppiness.}
      \vspace{0.2cm}
    ...
    \column{0.46\textwidth}
      ...
      \smallsource{ChatGPT Agent Mode screenshot via BKA Content (2024). Replace with an institutional capture for distribution.}
  \end{columns}
\end{frame}

% -------- Slide 3: Desktop agent apps --------
\begin{frame}{Desktop agent apps: the IDE boundary is blurring}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.48\textwidth}
      ...
      \begin{itemize}\setlength\itemsep{0.3em}
        \item IDEs are not disappearing overnight, but their core functions are being absorbed by agent apps.
        \item The old distinction ``chat app vs IDE'' is becoming weaker.
        \item For researchers, the relevant question is: \emph{can I review what changed?}
      \end{itemize}
  \end{columns}
  \vspace{0.4cm}
  \midpoint{The interface is moving from editor-first to agent-first.}
\end{frame}
...
\begin{frame}{3. IDE agents}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \threebullets%
        {Cursor, GitHub Copilot Agent, Windsurf, Antigravity, Codex inside competing IDEs, JetBrains AI, VS Code Copilot Chat.}%
      ...
      \begin{itemize}\setlength\itemsep{0.25em}
        ...
        \item New desktop agent apps increasingly do the same work outside a traditional IDE.
      \end{itemize}
    \column{0.48\textwidth}
