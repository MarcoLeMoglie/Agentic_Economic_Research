...
\begin{frame}{Subscription vs API}
  ...
  \smallsource{Pricing pages from OpenAI, Anthropic, Google, Cursor \citep{openai_pricing,anthropic_pricing,google_ai_plans,cursor_pricing}. Verify before teaching.}
\end{frame}
    ...
    \column{0.48\textwidth}
      \begin{tikzpicture}[every node/.style={align=center,font=\small}]
        \node[draw=deepblue,fill=softblue,rounded corners,very thick,
              ...
              (ctx){\textbf{Context size}\\\scriptsize tokens \emph{in}};
        \node[draw=accent,fill=softorange,rounded corners,very thick,
              minimum width=2.6cm,minimum height=0.9cm,right=0.5cm of ctx]
              (calls){\textbf{Tool calls}\\\scriptsize \# of round-trips};
        \node[draw=deepblue,fill=softgreen,rounded corners,very thick,
              minimum width=2.6cm,minimum height=0.9cm,below=0.6cm of ctx]
              (ret){\textbf{Retries}\\\scriptsize errors, self-checks};
        \node[draw=accent,fill=softpurple,rounded corners,very thick,
              minimum width=2.6cm,minimum height=0.9cm,below=0.6cm of calls]
              (mod){\textbf{Model tier}\\\scriptsize \$ per million tok.};
        \node[draw=dark,fill=white,line width=1pt,rounded corners,
              minimum width=2.6cm,minimum height=0.9cm,
              below=2cm of $(ctx)!0.5!(calls)$]
              (cost){\textbf{\color{deepblue}\Large cost}};
        \draw[->,thick,color=muted] (ctx.south)--(cost.north west);
        \draw[->,thick,color=muted] (calls.south)--(cost.north east);
        \draw[->,thick,color=muted] (ret.east)--(cost.west);
        \draw[->,thick,color=muted] (mod.west)--(cost.east);
      \end{tikzpicture}
    \column{0.48\textwidth}
      \fourbullets%
